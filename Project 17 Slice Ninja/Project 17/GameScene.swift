...
//  Project 17
//
//  Created by LiangTzu Chun on 3/16/16.
...
class GameScene: SKScene {
    ...
    override func touchesMoved(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        for node in nodes {
            if node.name == "enemy" {
                ...
                addChild(emitter)
                let re = SKAction.waitForDuration(1)
                let remove = SKAction.removeFromParent()
                let emitterSeq = SKAction.sequence([re , remove])
                emitter.runAction(emitterSeq)
    ...
    func endGame(triggeredByBomb triggeredByBomb: Bool) {
        ...
        playAgain.position = CGPoint(x: 300, y: 400)
    }
